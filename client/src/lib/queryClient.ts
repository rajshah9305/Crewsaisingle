...
async function throwIfResNotOk(res: Response) {
  if (!res.ok) {
    ...
    if (contentType && contentType.includes("application/json")) {
      try {
        errorData = await res.json() as { error?: string; [key: string]: unknown };
      } catch {
    ...
    (error as Error & { response?: { data: unknown; status: number } }).response = { data: errorData, status: res.status };
    throw error;
  }
}
