...
let poolInitializationPromise: Promise<pg.Pool> | null = null;
...
/**
 * Get database connection pool with retry logic and race condition protection
 */
async function getPool(retry = 0): Promise<pg.Pool> {
  ...
  poolInitializationPromise = (async (): Promise<pg.Pool> => {
    try {
      pool = createPool();
      // Test connection - pool is guaranteed to be non-null here
      ...
      // Reset promise on final failure
      poolInitializationPromise = null;
...
export class DatabaseStorage implements IStorage {
  ...
  async checkHealth(): Promise<boolean> {
    try {
      ...
      // Reset pool on health check failure to allow reconnection
      if (pool) {
        try {
          await pool.end();
        } catch (endError) {
          logger.error('Error closing pool during health check failure', {
